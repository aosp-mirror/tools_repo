...
class XmlManifest(object):
    ...
    def Link(self, name):
        ...
        try:
            ...
            raise ManifestParseError(
                'cannot link manifest %s: %s' % (name, str(e)))
    ...
    def Save(self, fd, peg_rev=False, peg_rev_upstream=True, groups=None):
        ...
        def output_project(parent, parent_node, p):
            ...
            if peg_rev:
                ...
                revision = self.remotes[
                    p.remote.orig_name].revision or d.revisionExpr
    ...
    def _Load(self):
        if not self._loaded:
            ...
            try:
                ...
                # this error and try again later, we will show the correct
                # error
                self._Unload()
                raise e
    ...
    def _AddMetaProjectMirror(self, m):
        ...
        if name is None:
            ...
            manifestUrl = self.manifestProject.config.GetString(
                'remote.origin.url')
            remote = _XmlRemote('origin', fetch=m_url[
    ...
    def GetProjectPaths(self, name, path):
        ...
        if self.IsMirror:
            ...
            objdir = os.path.join(
                self.repodir, 'project-objects', '%s.git' % name)
